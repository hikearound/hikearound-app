...
function GooglePlacesSearch({ theme, onPress, placeholder }) {
    ...
    try {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(
          ...
          Constants.expoConfig.extra.googlePlaces.apiKey
        }`
      );
  ...
  const handleSelectPlace = useCallback(
    (placeId, description) => {
      ...
      fetch(
        `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=formatted_address,geometry&key=${Constants.expoConfig.extra.googlePlaces.apiKey}`
      )
