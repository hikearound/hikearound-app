...
export function writeReviewLikes(rid, userLikes) {
  return db.collection('reviews').doc(rid).set({ userLikes }, { merge: true });
}

export function removeReviewFromList(reviews, selectedReview) {
  const deletedReview = reviews.find(review => review.id === selectedReview);
...
export async function queryReviews(
  ...
  let sortedReviews = Object.values(data).sort((a, b) => a.savedOn < b.savedOn);
